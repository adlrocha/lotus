...
func retrieval(t *testkit.TestEnvironment) error {
	...
	retrievalInfo := &testkit.RetrievalInfo{}
	// 1MB data file. We should make this configurable.
	data := make([]byte, 1000000)

	t.RecordMessage("THIS IS MY GROUPSEQ", t.GroupSeq)
	if t.GroupSeq == 1 {
		t.RecordMessage("I am the first client, I am making the deal with the miner!")
		...
		t.RecordMessage("client file cid: %s", fcid)
		...
		t.RecordMessage("Sending retrievalInfo")
		retrievalInfo = &testkit.RetrievalInfo{fcid.Root, minerAddr}
		t.SyncClient.Publish(ctx, testkit.RetrievalInfoTopic, retrievalInfo)

	} else {
		t.RecordMessage("Gather retrievalInfo")
		infoCh := make(chan *testkit.RetrievalInfo, 1)
		sctx, cancel := context.WithCancel(ctx)
		defer cancel()
		t.SyncClient.MustSubscribe(sctx, testkit.RetrievalInfoTopic, infoCh)
		retrievalInfo = <-infoCh
	}

	minerAddr := retrievalInfo.MinerAddr
	fcid := retrievalInfo.ContentCid
	...
	for i := 0; i < runCount; i++ {
		t.RecordMessage("Starting retrieval #%d for %s", i, fcid)
		t1 := time.Now()
		_ = testkit.RetrieveData(t, ctx, client, fcid, nil, carExport, data)
		t.R().RecordPoint(fmt.Sprintf("deal.retrieved/fast=%v", fastRetrieval), float64(time.Since(t1).Milliseconds()))
